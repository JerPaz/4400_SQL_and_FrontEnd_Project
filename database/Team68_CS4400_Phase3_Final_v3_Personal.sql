...
INSERT INTO cs4400spring2020.building(buildingName, description) VALUES ("Test_Building", "This is a building strictly for test");
INSERT INTO cs4400spring2020.Station(stationName, buildingName, capacity) VALUES ("Test_Station", "Test_Building", 50);
...
-- Make sure you check password length >= 8 (STILL NEED TO CHECK)
...
BEGIN
	IF length(i_password) >= 8 AND (i_balance > 0 OR length(i_email) > 0) THEN
		...
		IF i_balance > 0 THEN
			INSERT INTO cs4400spring2020.Customer(username, balance, stationName) VALUES (i_username, i_balance, NULL);
		END IF;
        
		IF i_type = "Manager" THEN
			INSERT INTO cs4400spring2020.Employee(username, email) VALUES (i_username, i_email);
			INSERT INTO cs4400spring2020.Manager(username) VALUES (i_username);
		ELSEIF i_type = "Admin" THEN
			INSERT INTO cs4400spring2020.Employee(username, email) VALUES (i_username, i_email);
			INSERT INTO cs4400spring2020.`Admin`(username) VALUES (i_username);
		ELSEIF i_type = "Staff" THEN
			INSERT INTO cs4400spring2020.Employee(username, email) VALUES (i_username, i_email);
			INSERT INTO cs4400spring2020.Staff(username) VALUES (i_username);
		END IF;
	END IF;
...
SELECT FoodTruck.foodTruckName, Station.stationName,  (Station.capacity - (SELECT COUNT(DISTINCT(FoodTruck.foodtruckName)) FROM FoodTruck WHERE FoodTruck.stationName = Station.stationName)) as remainingCapacity, COUNT(DISTINCT(Staff.username)) as staffCount, COUNT(DISTINCT(foodName)) as menuItemCount
    FROM FoodTruck
    INNER JOIN STATION
    ...
    (i_foodTruckName LIKE CONCAT("%", FoodTruck.foodTruckName,"%") OR i_foodTruckName = "" OR i_foodTruckName IS NULL) AND
    (i_stationName = Station.stationName OR i_stationName = "" OR i_stationName IS NULL) AND
    ((i_hasRemainingCapacity = TRUE AND (Station.capacity - (SELECT COUNT(DISTINCT(FoodTruck.foodTruckName)) FROM FoodTruck WHERE FoodTruck.stationName = Station.stationName))>0) OR (i_hasRemainingCapacity = FALSE))
...
BEGIN
	...
	DROP TABLE IF EXISTS mn_view_foodTruck_available_staff_result;
     ...
     FROM cs4400spring2020.`User`
     INNER JOIN STAFF
     ON STAFF.username = USER.username
     WHERE
     (staff.FoodTruckName = NULL) OR (staff.FoodTruckName = "");
...
-- Changed Procedure title based on Starting_Shell_v4 (original came from v3)
...
BEGIN
    ...
    SELECT Station.stationName, Station.buildingName, GROUP_CONCAT(DISTINCT(FoodTruck.foodTruckName)) AS foodTruckNames, GROUP_CONCAT(DISTINCT(MenuItem.foodName)) AS foodNames
        FROM Station
        INNER JOIN
        ...
        (i_foodName is NULL OR i_foodName = '' OR MenuItem.foodTruckName in 
        (SELECT FoodTruckName FROM MenuItem WHERE (MenuItem.foodName LIKE CONCAT('%', i_foodName, '%'))))
...
BEGIN
    ...
    VALUES(
		(SELECT stationName FROM cs4400spring2020.Customer WHERE username = i_customerUsername),
        
        (SELECT buildingName FROM cs4400spring2020.Station WHERE stationName in 
        (SELECT stationName FROM cs4400spring2020.Customer WHERE username = i_customerUsername)),
        
        (SELECT GROUP_CONCAT(tag) FROM cs4400spring2020.BuildingTag WHERE buildingName in (SELECT buildingName FROM cs4400spring2020.Station WHERE stationName in 
        (SELECT stationName FROM cs4400spring2020.Customer WHERE username = i_customerUsername))),
        
        (SELECT `description` FROM cs4400spring2020.Building WHERE buildingName in (SELECT buildingName FROM cs4400spring2020.Station WHERE stationName in 
        (SELECT stationName FROM cs4400spring2020.Customer WHERE username = i_customerUsername))),
        
        (SELECT balance FROM cs4400spring2020.Customer WHERE username = i_customerUsername)
        );
...
BEGIN
	...
	SELECT FoodTruck.foodTruckName, CONCAT(`User`.firstName, " ", `User`.lastName) as managerName, GROUP_CONCAT(DISTINCT(foodName)) as foodNames
		FROM (SELECT * FROM Customer WHERE Customer.username = i_customerUsername) as `alex`
		INNER JOIN 
        FoodTruck on `alex`.stationName = FoodTruck.stationName
        INNER JOIN
        `User` ON FoodTruck.managerUsername = `User`.username
        INNER JOIN
        MenuItem ON MenuItem.foodTruckName = FoodTruck.foodTruckName
        GROUP BY FoodTruck.foodTruckName;  
...
BEGIN
	INSERT INTO cs4400spring2020.Orders(date, customerUsername)
...
BEGIN
    ...
    IF ((SELECT balance FROM Customer WHERE Customer.username = (SELECT DISTINCT(customerUsername) FROM Orders WHERE i_orderID = orderID)) >= 
    (i_purchaseQuantity * (SELECT price FROM MenuItem WHERE i_foodName = MenuItem.foodName AND i_foodTruckName = MenuItem.FoodTruckName))) 
    THEN
    ...
    UPDATE Customer
    SET balance = balance - (i_purchaseQuantity * (SELECT price FROM MenuItem WHERE i_foodName = MenuItem.foodName AND i_foodTruckName = MenuItem.FoodTruckName))
    WHERE Customer.username = (SELECT DISTINCT(customerUsername) FROM Orders WHERE i_orderID = orderID);
    END IF;
...
BEGIN
    ...
    SELECT `date` , Orders.orderID, sum(OrderDetail.purchaseQuantity * MenuItem.price) as orderTotal, GROUP_CONCAT(DISTINCT(OrderDetail.foodName)) as foodNames, sum(purchaseQuantity) as foodQuantity
		FROM Orders
		LEFT JOIN
		OrderDetail ON Orders.orderID = OrderDetail.orderID
		LEFT JOIN
